...
pub_data <- read_csv("1_data/unibas_diversity_noduplicates.csv")
...
<img src="3_figures/header.png" width="80%"></img>
...
# pA1 = PERCENTAGE OF FEMALE AUTHORS (2000 - 2020)
# pA2 = PERCENTAGE OF FEMALE AUTHORS: FIRST VS. LAST (2000 - 2020)

# pB1 = PERCENTAGE OF FEMALE FACULTY (2013 - 2020)
# pB2 = PERCENTAGE OF FEMALES EMPLOYED: ACADEMIC STAFF VS. PROFESSORSHIPS (2013 - 2020)

# pC1 = PERCENTAGE OF FEMALES FUNDED: GENERAL (2008 - 2020)
# pC2 = PERCENTAGE OF FEMALES FUNDED: CLINICAL (2010 - 2020)
...
prop_fem_total <- pub_data %>%
  ungroup() %>% 
  summarise(prop_female = 100*mean(gender_proportion, na.rm = TRUE))

prop_fem_total_year <- pub_data %>%
  group_by(year) %>% 
  filter(year %in% c(2020, 2000)) %>%
  summarise(prop_female = 100*mean(gender_proportion, na.rm = TRUE))

prop_fem_inst_year <- pub_data %>%
select(year, gender_proportion, `Org-Swiss Tropical and Public Health Institute`: `Org-Basel Institute on Governance`) %>%
  pivot_longer(!year:gender_proportion,names_to = "organisation_name", values_to = "applies") %>% 
  filter(applies == 1) %>% #only select those rows that could be classified
  mutate(organisation_name = case_when(organisation_name == "Org-Faculty of Theology" ~ "Theology",
                                       organisation_name == "Org-Faculty of Science" ~ "Natural Sciences",
                                       organisation_name == "Org-Faculty of Law" ~ "Law",
                                       organisation_name == "Org-Faculty of Humanties and Social Sciences" ~ "Humanities & Social Sciences",
                                       organisation_name == "Org-Faculty of Medicine"  ~ "Medicine",
                                       organisation_name == "Org-Faculty of Business and Economics" ~ "Business & Economics",
                                       organisation_name == "Org-Faculty of Psychology" ~ "Psychology")) %>% 
  filter(!is.na(organisation_name)) %>% # ignoring non-faculty organizations
  group_by(organisation_name, year) %>% 
  summarise(n=n(),prop_female = 100*mean(gender_proportion, na.rm = TRUE))

# calc diff 2020-2000
prop_fem_inst_diff <- prop_fem_inst_year %>%
  filter(year %in% c(2020, 2000)) %>%
  group_by(year, organisation_name) %>%
  summarise(prop_female = mean(prop_female, na.rm = TRUE)) %>% 
  group_by(organisation_name) %>% 
  mutate(diff = diff(prop_female)) 

# total N publications
prop_fem_inst_n <- prop_fem_inst_year %>% 
  summarise(n = sum(n, na.rm = TRUE))

# total % females by faculty
prop_fem_inst <- prop_fem_inst_year %>%
  group_by(organisation_name) %>%
  summarise(prop_female = mean(prop_female, na.rm = TRUE))

# put tables together
table<-prop_fem_inst_diff %>% 
  pivot_wider(organisation_name,names_from = year,values_from = c(prop_female,diff)) %>% 
  select(-diff_2000)

table<-left_join(table,prop_fem_inst_n,by="organisation_name") # add n
  
table<-left_join(table,prop_fem_inst,by="organisation_name") %>%  # add 
  select(organisation_name,n,prop_female,prop_female_2000,prop_female_2020,diff_2020) %>% 
  mutate(across(where(is.numeric), round, 0))

names(table)<-c("Faculty","Pubs","Total % Female","2000 % Female", "2020 % Female", "Diff %")
...
prop_fem_employ <-  mean(gender_inst_time$FEMALE, na.rm = TRUE)


```

- The figure above shows evidence of a publication gender gap at the University of Basel. Across all faculties and time, the predominance of the dark blue color indicates there were overall more publications involving males (`r 100-round(prop_fem_total$prop_female, 0)`%) relative to females (`r round(prop_fem_total$prop_female, 0)`%). Notwithstanding, the figure suggests a general upward trend in the percentage of publications involving females over the last two decades, while in 2000 the percentage of female authors was `r round(prop_fem_total_year$prop_female[1], 0)`%, in 2020 this percentage was `r round(prop_fem_total_year$prop_female[2], 0)`%.

- The overall levels as well as temporal trends vary, however, markedly between faculties: some faculties show a greater percentage of publications involving females across the full 20-year period (2000-2020), most notably the Faculty of Psychology (`r round(prop_fem_inst$prop_female[prop_fem_inst_nyr$organisation_name == "Psychology"], 0)`%), the Faculty of Humanities and Social Sciences (`r round(prop_fem_inst$prop_female[prop_fem_inst_nyr$organisation_name == "Humanities &\nSocial Sciences"], 0)`%), and the Faculty of Law (`r round(prop_fem_inst$prop_female[prop_fem_inst_nyr$organisation_name == "Law"], 0)`%). Concerning temporal trends, some faculties have seen a greater increase, most notably the Faculty of Psychology, which saw an increase of `r round(prop_fem_inst_diff$diff[prop_fem_inst_diff$organisation_name == "Psychology" & prop_fem_inst_diff$year == 2000], 0)`% from 2000 to 2020. The following table shows the total number of publications, the overal percentage of publications involving females, as well as a breakdown for 2000 vs. 2020, along with the difference between the two. 
...
First authorship is typically (but not always) associated with junior academic roles in publications involving multiple authors, while last authorship typically cues seniority. Consequently, distinguishing between first and last authorship can allow us to consider trends in the publication gender gap as a function of seniority (i.e., career stage). When considering these analysis and, particularly, comparing between faculties, it should be noted that determination of authorship order follows different practices across fields (e.g., alphabetical order in economics). Below, we show the breakdown of the publication gap by first vs. last author for publications involving multiple authors (upper panel) along with the percentage employment of females in professorships vs. academic staff (lower panel). Please note that we only present employment data for the 2013-2019 period (available from the Equal Opportunities Monitoring) and extrapolate the temporal pattern for the full 2000-2020 period using a statistical model (linear regression). 

```{r, echo=FALSE, eval=TRUE, fig.width = 20, fig.height= 28}


fig_authors_staff <- pB1/pA2/pB2
fig_authors_staff 

```

- As can be seen in the upper panel, across nearly all faculties and time, the predominance of the dark blue color indicates there were overall more males (`r 100 - round(prop_fem_employ, 0)`%) relative to females (`r round(prop_fem_employ, 0)`%) employed as researchers.

- As can be seen in the middle panel, there is a general trend for larger proportion of females in first vs. last authors; this pattern matches the larger prevalence of females in junior (academic staff) vs. senior (professorships) roles that is evident in the lower panel. 

- As presented in the previous section concerning the overall percentage of females, the temporal trends split by first vs. last author show similar increases over time, and, again, patterns differ somewhat betwen faculties. Crucially, the temporal trends observed are not always parallel for first and last authors suggesting that the dynamics underlying academic productivity can differ somewhat for the different career stages, at least for some faculties (cf. Natural Sciences, Psychology, Theology). 


## Linking the Publication and Employment Gender Gaps: The Role of Funding
...
- We use faculty as organizational unit but some faculties are composed of an htereogenous number of departments covering different academic fields that may differ from each other in several respects (e.g., Faculty of Natural Sciences). Future work could conduct more specific analysis per department or academic field, albeit smaller number of publications per organizational unit may present a limiting factor for such analyses. 

- We cannot guarantee that the research database we relied upoon provides full and equal coverage of all researchers of the University of Basel. Future work could include the use of additional databases (e.g., Dimensions, Scopus, Web of Science) to assess the current coverage of the research database we used. 

- We used authorship order (i.e., first vs. last author) as a proxy for seniority, however, this practice is deferentially prevalent across fields and time. Using admistrative data could be helpful in assessing the role of seniority on academic productivity and the publication gap at the University of Basel. 

- Our report identifies a gender publication gap but does not address potential underlying causes beyond differences in employment. Additional work could involve using additional monitoring (see Outlook) or adopting other approaches such as interviews or targeted questionnaires involving specific groups of researchers to investigate the underlying causes leading to a publication gap. 
...
- Finally, we analyzed only one dimension of diversity: Gender. The use of similar approaches using other services that  categorize other individual characteristics could, however, be used in the future to complement the diversity monitoring of the University of Basel, for example, regarding country of origin or ethnicity (e.g., [https://www.namsor.com](https://www.namsor.com)). 
...
The flow diagram above presents a framework destilled from the existing literature (e.g.Huang et al., 2020; Ni et al. 2021, Odic & Woicik, 2020) and lists factors that may contribute to a widening gap between employment and academic output. 

<img src="3_figures/leakypipeline.png" width="80%"></img>

There are a few recommendations that result from the framework above. 

- The University of Basel could expand its monitoring to assess inequalities in research productivity and its associated factors, specifically the university could expand current monitoring of financial support to academic staff to include financial amounts to males vs. females (cf.  [Career Advancement Activity Reports](https://www.unibas.ch/de/Universitaet/Administration-Services/Vizerektorat-Forschung/Nachwuchsfoerderung/Taetigkeitsberichte.html)) and additional monitoring of internal funding at the professorial level (startup, support staff, lab space). 

- The University of Basel could be interested in linking academic productivity with individual characteristics that are not currently available for analysis (e.g., family status, childcare). There are several challenges associated with this approach, including issues of privacy, but analyses of academic output we present above using such variables have the potential to guide decisions concerning the role of personal characteristics (e.g., family responsibilities) that could receive specific attention by higher-education institutions. 

- Despite our documentation of employment and publication gaps, one should acknoweldge that the University of Basel already has in place a number of programs to counteract these effects (e.g., Antelope). These programs are recent and likely to exert their effects over a time beyond the period we analyzed. Further, while our results support the claim of a (universally) larger publication relative to an employment gaps, the proportion of female authors seems to be increasing more strongly than female employment, which could be indicative of intervention success. A broader focus on program evaluation, including increases in academic productivity could be an important focus for future work on reducing inequalities at the University of Basel.

## References 

Huang, J., Gates, A. J., Sinatra, R., & Barabási, A.-L. (2020). Historical comparison of gender inequality in scientific careers across countries and disciplines. *Proceedings of the National Academy of Sciences of the United States of America, 117*(9), 4609–4616. [http://doi.org/10.1073/pnas.1914221117](http://doi.org/10.1073/pnas.1914221117)

Ni, C., Smith, E., Yuan, H., Larivière, V., & Sugimoto, C. R. (2021). The gendered nature of authorship. *Science Advances, 7*(36), eabe4639. [http://doi.org/10.1126/sciadv.abe4639](http://doi.org/10.1126/sciadv.abe4639)
